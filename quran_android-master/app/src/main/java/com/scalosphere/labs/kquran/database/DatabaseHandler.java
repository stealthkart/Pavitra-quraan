...
public class DatabaseHandler {
  ...
  public static String COL_FOOTNOTES = "footnotes";
  ...
  public DatabaseHandler(Context context, String databaseName)
    ...
    try {
     ...
     // Crashlytics.log("database file " + path + (new File(path).exists()? " exists" : " doesn't exist"));
      throw se;
    }
    ...
    public DatabaseHandler(Context context, String databaseName, boolean baseDir)
            throws SQLException {
        String base ="";
        if(baseDir){
            base = QuranFileUtils.getQuranBaseDirectory(context);
        }
       ...
       // Crashlytics.log("opening database file: " + path);
        ...
        mSchemaVersion = getSchemaVersion();
        mDatabasePath = path;
        //mMatchString = "<font color=\"" + context.getResources().getColor(R.color.translation_highlight) + "\">";
    }
  ...
  public Cursor getVerses(int sura, int minAyah, int maxAyah) {
    ...
    //TODO for Kannada Qur'an initial schema version will be 2 as we would like to have full text search feature enabled.
  public int getSchemaVersion() {
   ...
   //TODO for Kannada Qur'an initial text version will be 1 and we can increment it as we need to make changes in the translation content in the server
...
/*  public Cursor getNotes(int minSura, int minAyah, int maxSura,
  ...
  public Cursor getVerses(int minSura, int minAyah, int maxSura,
    ...
    whereQuery.append(")");
      if(table.equals(VERSE_TABLE)){ // Reading from Kannada Database
          return mDatabase.query(table,
                  new String[]{COL_SURA, COL_AYAH, COL_TEXT,COL_FOOTNOTES},
                  whereQuery.toString(), null, null, null,
                  COL_SURA + "," + COL_AYAH);
      }else{ //// Reading from Arabic Database
          return mDatabase.query(table,
  ...
  public Cursor search(String q, String table, boolean withSnippets) {
    ...
    //Log.i("DBHandler", " useFullTextIndex is " + useFullTextIndex);
    if (useFullTextIndex) {
      operator = " MATCH ";
      query = query + "*";
    } else {
    ...
    String qtext = "select " + COL_SURA + ", " + COL_AYAH +
        ...
        operator + " ? " + " limit 150";
    //Log.i("DBHandler","search query: " + qtext + ", query: " + query);
    ...
    try {
      ...
      //Log.i("DbHandler" , " c.getCount()" + c.getCount());
