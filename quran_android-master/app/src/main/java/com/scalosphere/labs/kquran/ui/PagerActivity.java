...
import com.scalosphere.labs.kquran.data.AyahInfoDatabaseHandler;
...
import com.scalosphere.labs.kquran.database.TranslationsDBAdapter;
...
import com.scalosphere.labs.kquran.task.RefreshBookmarkIconTask;
import com.scalosphere.labs.kquran.task.ShareAyahTask;
import com.scalosphere.labs.kquran.ui.fragment.AddTagDialog;
import com.scalosphere.labs.kquran.ui.fragment.AyahActionFragment;
import com.scalosphere.labs.kquran.ui.fragment.FontMenuFragment;
...
import com.scalosphere.labs.kquran.ui.fragment.SeekbarFragment;
...
import com.scalosphere.labs.kquran.ui.helpers.AyahSelectedListener;
...
import com.scalosphere.labs.kquran.ui.helpers.QuranPageAdapter;
import com.scalosphere.labs.kquran.ui.helpers.QuranPageWorker;
import com.scalosphere.labs.kquran.ui.helpers.SlidingPagerAdapter;
...
import com.scalosphere.labs.kquran.widgets.AudioStatusBar;
import com.scalosphere.labs.kquran.widgets.AyahToolBar;
import com.scalosphere.labs.kquran.widgets.IconPageIndicator;
import com.scalosphere.labs.kquran.widgets.SlidingUpPanelLayout;
...
public class PagerActivity extends AppCompatActivity implements
  ...
  public static final String EXTRA_UNHIGHLIGHT_SURA = "unhighlightSura";
  public static final String EXTRA_UNHIGHLIGHT_AYAH = "unhighlightAyah";
  ...
  // TODO Change mShowingTranslation flag to show Translantion first

  private boolean mShowingTranslation = true;
  private int mHighlightedSura = -1;
  private int mHighlightedAyah = -1;

  private int mSuraToUnHighlight = -1;
  private int mAyahToUnHighlight = -1;
  ...
  private boolean twolangauges = true;
  ...
  //private FootnotesPopup popupDialog = new FootnotesPopup();
  private static Map<Integer,List<String>> notes;
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    mSuraToUnHighlight = extras.getInt(EXTRA_UNHIGHLIGHT_SURA, -1);
    mAyahToUnHighlight = extras.getInt(EXTRA_UNHIGHLIGHT_AYAH, -1);
}
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
      setUiVisibilityListener();
       ...
       // Scalo TO disable Footer (Audio) set view as INVISIBLE
      //mAudioStatusBar.setVisibility(View.INVISIBLE);
    }
      ...
      mHandler.post(new Runnable() {
        ...
        public void run() {
          ...
          if (mDualPages) {
            ...
            page = Constants.PAGES_LAST_DUAL - (page / 2);
          } else {
            if (page % 2 == 0) {
              page--;
            }
  ...
  public void onResume() {
    ...
    if (mHighlightedSura > 0 && mHighlightedAyah > 0) {
      ...
      if (mSuraToUnHighlight > 0 && mAyahToUnHighlight > 0) {
          mHandler.postDelayed(
                  new Runnable() {
                      public void run() {
                          highlightAyah(mSuraToUnHighlight, mAyahToUnHighlight, false, HighlightType.BLACK);
                      }
                  }, 750);
      }
  }
  ...
  public void downloadRequiredFiles() {
    ...
    if (!QuranFileUtils.hasArabicSearchDatabase(this)) {
      ...
      View layout = inflater.inflate(R.layout.toast,(ViewGroup) this.findViewById(R.id.toast_layout_root));
    ...
    if (downloadType != QuranDownloadService.DOWNLOAD_TYPE_AUDIO) {
      // if audio is playing, just show a status notification
        text.setText(R.string.downloading_title);
        toast.show();
...
//TODO check this flag for showing translation directly
  ...
  public boolean onPrepareOptionsMenu(Menu menu) {
    ...
    MenuItem kannada = menu.findItem(R.id.goto_kannada);
    MenuItem foot_notes = menu.findItem(R.id.foot_notes);
    MenuItem fontSize = menu.findItem(R.id.change_text_size);
    MenuItem fontMenu = menu.findItem(R.id.font_menu);

    if (!mShowingTranslation) { //Showing Arabic Qur'aan pages
      quran.setVisible(false);
      translation.setVisible(true);
      kannada.setVisible(false);
      foot_notes.setVisible(false);
      fontSize.setVisible(false);
      fontMenu.setVisible(false);
    } else if(mShowingTranslation && twolangauges) {
        quran.setVisible(true);
        translation.setVisible(false);
        kannada.setVisible(true);
        foot_notes.setVisible(true);
...
/*
    Log.i(TAG, " kannada.isVisible()"+kannada.isVisible());
    Log.i(TAG, " translation.isVisible()"+translation.isVisible());
    Log.i(TAG, " quran.isVisible()"+quran.isVisible());
*/
...
//    nightMode.setChecked(prefs.getBoolean(Constants.PREF_NIGHT_MODE, false));
  ...
  public static void setNotes(int page, List<String> noteList){
      if(notes!=null && !notes.containsKey(page)){
          notes.put(page,noteList);
          //Log.i(TAG, " Notes in a map "+ notes);
      }else if(notes==null){
          notes= new HashMap<Integer, List<String>>();
          notes.put(page,noteList);
      }else{
         // Log.i(TAG, " Notes in a map in else "+ notes);
      }

  }

    public static boolean hasNotes(int page) {
        if(notes!=null) {
            return notes.containsKey(page);
        }else{
            return false;
        }
    }

  public static List<String> getNotes(int page) {
      if(notes!=null && notes.containsKey(page)) {
          return notes.get(page);
      }else{
          //Log.i(TAG, " No list is available for page "+page);
          return null;
      }
  }
  ...
  public boolean onOptionsItemSelected(MenuItem item) {
    ...
    if (itemId == R.id.favorite_item) {
      ...
      switchToTranslation();
      return true;
    } else if (itemId == R.id.goto_kannada) {
      twolangauges = false;
      switchToKannada();
      return true;
        ...
        gotoSeekBarDialog();
        return true;
    }else if (item.getItemId() == R.id.font_menu){
        gotoFontDialog();
        return true;
    } else if (itemId == R.id.foot_notes) {
        //Log.i(TAG, "Foot notes for the current page " + getCurrentPage()+ " are "+getNotes(getCurrentPage()));
        FragmentManager fm = getSupportFragmentManager();
        FootnotesPopup popupDialog = FootnotesPopup.newInstance(getCurrentPage());
        popupDialog.show(fm, FootnotesPopup.TAG);
        return true;
...
//      return true;
  ...
  public void refreshTranslationPages(){
      if(twolangauges){
          switchToQuran();
          switchToTranslation();
      }else{
      ...
      //Log.i(TAG, " switched to respective view - twolanguages flag is " + twolangauges  );
  }
  ...
  public void switchToQuran() {
    ...
    public void switchToKannada() {
        if (mIsInAyahMode) {
            endAyahMode();
        }
        String activeDatabase = TranslationUtils.getDefaultTranslation(this, mTranslations);
        if (activeDatabase == null) { // we should make sure that activeDatabase is never null
            //TODO need to comment this and null has to be handled by downloading the Kannada DB
            //
            //startTranslationManager(); //this will not be started
        } else {
            mPagerAdapter.setKannadaMode();
            mShowingTranslation = true;
            QuranSettings.setTwoLanguageView(false);
            invalidateOptionsMenu();
            updateActionBarSpinner();
        }
    }

    public void gotoFontDialog() {
        FragmentManager fm = getSupportFragmentManager();
        FontMenuFragment fontMenuDialog = new FontMenuFragment();
        fontMenuDialog.show(fm, fontMenuDialog.TAG);
    }
...
/*
    ...
    public static void setTranslationTextSize(int fontSize) {
        if(fontSize <Constants.DEFAULT_TEXT_SIZE){
...
*/
  ...
  public void switchToTranslation() {
    ...
    if (activeDatabase == null) { // we should make sure that activeDatabase is never null
     ...
     //startTranslationManager(); //this will not be started
      ...
      QuranSettings.setTwoLanguageView(true);
      mPagerAdapter.setTranslationMode();
      mShowingTranslation = true;
      invalidateOptionsMenu();
      updateActionBarSpinner();
 ...
 /* public void startTranslationManager() {
  ...
  //TODO the below code for Translation Item navigation has been disabled for KQD
  ...
  public void onTagAdded(final String name) {
    if (mIsInAyahMode) {
      if (TextUtils.isEmpty(name))
        return;
      TagBookmarkDialog f = (TagBookmarkDialog) mSlidingPagerAdapter.getFragmentIfExists(SlidingPagerAdapter.TAG_PAGE);
      if (f != null) {
...
//    actionBar.setNavigationMode(ActionBar.NAVIGATION_MODE_STANDARD);
    ...
    actionBar.setSubtitle(desc);
      ...
      // TODO added to disable ActionBar Title and setSubtitle.
  ...
  public void highlightAyah(int sura, int ayah,
    ...
    if (f != null && f instanceof AyahTracker) {
    if(f instanceof QuranPageFragment && type == HighlightType.BLACK ){
        return;
    }
    ((AyahTracker) f).highlightAyah(sura, ayah, type);
      //Log.i(TAG, "f.getClass()"+ f.getClass() ) ;
    }
  }
  ...
  class TranslationReaderTask extends AsyncTask<Void, Void, Void> {
    ...
    protected Void doInBackground(Void... params) {
      try {
        ...
        Log.e(TAG, "error getting translations list", e);
      }
      return null;
    }
  ...
  public void onPlayPressed() {
    ...
    if (mDualPages) {
      page = ((Constants.PAGES_LAST_DUAL - position) * 2) - 1;
    }
...
/*    mNeedsPermissionToDownloadOver3g = true;
  ...
  public void onAudioSettingsPressed() {
    ...
    showSlider(SlidingPagerAdapter.AUDIO_PAGE);
  }
...
/*    startService(new Intent(AudioService.ACTION_STOP));
  ...
  private void refreshPages() {
    for (int page : SlidingPagerAdapter.PAGES) {
      if (page == SlidingPagerAdapter.TAG_PAGE) {
        TagBookmarkDialog tagsFrag =
            (TagBookmarkDialog) mSlidingPagerAdapter
                .getFragmentIfExists(SlidingPagerAdapter.TAG_PAGE);
        if (tagsFrag != null) {
