package com.scalosphere.labs.kquran.service;
...
public class QuranDownloadService extends Service {
   ...
   public static final String ACTION_DOWNLOAD_URL =
         "com.scalosphere.labs.kquran.DOWNLOAD_URL";
   public static final String ACTION_DOWNLOAD_RANGE =
         "com.scalosphere.labs.kquran.DOWNLOAD_RANGE";
   public static final String ACTION_CANCEL_DOWNLOADS =
         "com.scalosphere.labs.kquran.CANCEL_DOWNLOADS";
   public static final String ACTION_RECONNECT =
         "com.scalosphere.labs.kquran.RECONNECT";
   public static final String ACTION_CHECK_TRANSLATIONS =
         "com.scalosphere.labs.kquran.CHECK_TRANSLATIONS";

   private static final String UPGRADING_EXTENSION = ".ex";
   ...
   public static final String DB_DOWNLOAD_KEY = "db_downloadKey";
   ...
   public static final int DOWNLOAD_TYPE_KAN_SEARCH_DB = 5;
   ...
   public static class ProgressIntent {
      public static final String INTENT_NAME =
            "com.scalosphere.labs.kquran.download.ProgressUpdate";
   ...
   private void updateTranslations(){
       //todo  TRY TO UPDATE THIS CODE TO HANDLE KANNADA db UPGRADE
      ...
      for (TranslationItem item : items){
       ...
       /*Log.i (TAG, " needsUpgrade is "+ needsUpgrade);
   ...
   private void onHandleIntent(Intent intent){
      if (ACTION_DOWNLOAD_URL.equals(intent.getAction())){
        ...
        // Log.i(TAG," download url in onHandleIntent "+url);
         ...
         if (startAyah != null && endAyah != null){
           ...
           // Log.i(TAG," url "+ url + " dest "+ destination+"  outputFile " + outputFile + " details "+ details );
            result = download(url, destination, outputFile, details);
           // Log.i( TAG," download result is "+result);
         }
         if (result && isZipFile){
   ...
   private boolean download(String urlString, String destination,
      ...
      android.util.Log.i(TAG, "making directory: " + destination);
...
*/
   ...
   private int downloadFile(String urlString, String destination,
      ...
      try {         
         ...
         String filename = QuranDownloadService.getFilenameFromUrl(urlString);
         //Log.i(TAG, "getFilenameFromUrl "+filename);
         String fileToCheck = filename;
         if (outputFile != null){
             //Log.i(TAG, "outputFile  "+outputFile + "fileToCheck  "+fileToCheck );
         ...
         android.util.Log.i(TAG, "got content length: " + contentLength +
               ", rc: " + connection.getResponseCode());         

         File actualFile = new File(destination, fileToCheck);
         android.util.Log.i(TAG, "actualFile: " + actualFile.getPath() +
        ...
        //Log.i(TAG, "response code rc is  " + rc + " !actualFile.exists() == " +!actualFile.exists() + "fileLength = "+  fileLength + " actualFile.length() "+ actualFile.length());
          // Begin New logic to handle Kannada DB Upgrade when the Db is existing already
          try {
              if(actualFile.getName().equals(QuranDataProvider.QURAN_KAN_DATABASE)) {
                  File f = new File(QuranFileUtils.getQuranDatabaseDirectory(this),
                          actualFile + UPGRADING_EXTENSION);
                  if (f.exists()) {
                      //Log.i(TAG, "deleting " + f.getAbsolutePath());
                      f.delete();
                  }
                  if (actualFile.exists()) {
                      //Log.i(TAG, "deleting " + actualFile.getAbsolutePath());
                      actualFile.delete();
                  }
              }
          }
          catch (Exception e){
              Log.e(TAG, "error removing old database file", e);
          }
        ...
        // End new logic
         ...
         if (rc == HttpURLConnection.HTTP_PARTIAL &&
            ...
            int x = 0;
            int updateCount = 0;
            //Log.i(TAG, " mIsDownloadCanceled "+ mIsDownloadCanceled  + " downloaded " + downloaded );
            ...
            //Log.i(TAG, " outside while downloaded " + downloaded );
         }
         else if (rc != HttpURLConnection.HTTP_PARTIAL && rc != 416){
            Log.e(TAG, "got unexpected response code: " + rc);
            notifyError(ERROR_NETWORK, false, notificationInfo);
            return ERROR_NETWORK;
         }
         ...
         if (!mIsDownloadCanceled){
            notifyDownloadProcessing(notificationInfo, 0, 0);
             //Log.i(TAG, " notifyDownloadProcessing " + downloaded );
         }
         
         if (!actualFile.exists() && downloaded == fileLength){
            android.util.Log.i(TAG, "moving file...");
         ...
         if (actualFile.exists()){
            if (actualFile.getName().endsWith(".zip")){
               if (!unzipFile(actualFile.getAbsolutePath(), destination,
                ...
                //Log.i(TAG, " !actualFile.exists()  " + !actualFile.exists() + " downloaded == " +downloaded + "fileLength = "+  fileLength);
                //TODO Handle Kannada DBA upgrade
            }
