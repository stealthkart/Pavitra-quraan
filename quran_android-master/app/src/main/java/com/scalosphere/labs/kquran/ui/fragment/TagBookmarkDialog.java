...
public class TagBookmarkDialog extends SherlockDialogFragment {
  ...
  public class TagsAdapter extends BaseAdapter {
      ...
      public View getView(int position, View convertView,
         ...
         final BookmarksDBAdapter.Tag tag = (BookmarksDBAdapter.Tag)getItem(position);
   ...
   class RefreshTagsTask extends AsyncTask<Void, Void, ArrayList<BookmarksDBAdapter.Tag>> {
      @Override
      protected ArrayList<BookmarksDBAdapter.Tag> doInBackground(Void... params) {
         ...
         ArrayList<BookmarksDBAdapter.Tag> newTags = new ArrayList<BookmarksDBAdapter.Tag>();
         newTags.addAll(adapter.getTags());
         newTags.add(new BookmarksDBAdapter.Tag(-1, newTagString));
      ...
      @Override
      protected void onPostExecute(ArrayList<BookmarksDBAdapter.Tag> result) {
         if (result != null) {
            for (BookmarksDBAdapter.Tag tag : result) {
   ...
   class AddTagTask extends AsyncTask<String, Void, BookmarksDBAdapter.Tag> {
       @Override
       protected BookmarksDBAdapter.Tag doInBackground(String... params) {
          BookmarksDBAdapter adapter = null;
          Activity activity = getActivity();
          if (activity != null && activity instanceof BookmarkHandler){
             adapter = ((BookmarkHandler) activity).getBookmarksAdapter();
          ...
          return new BookmarksDBAdapter.Tag(id, params[0]);
       }

       @Override
       protected void onPostExecute(BookmarksDBAdapter.Tag result) {
