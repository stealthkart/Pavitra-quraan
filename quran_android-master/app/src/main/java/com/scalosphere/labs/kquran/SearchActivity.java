...
import com.scalosphere.labs.kquran.ui.TranslationManagerActivity;
...
public class SearchActivity extends FragmentActivity
  ...
  private boolean mDownloadKanSearchDb = false;
  private boolean mIsArabicSearch = false;
  private boolean mIsKanSearch = false;
  ...
  private void downloadArabicSearchDb() {
    ...
    public void downloadKanSearchDb() {
        if (mDownloadReceiver == null) {
            mDownloadReceiver = new DefaultDownloadReceiver(this,
                    QuranDownloadService.DOWNLOAD_TYPE_KAN_SEARCH_DB);
            LocalBroadcastManager.getInstance(this).registerReceiver(
                    mDownloadReceiver, new IntentFilter(
                            QuranDownloadService.ProgressIntent.INTENT_NAME));
        }
  ...
  public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
    ...
    mIsKanSearch = QuranUtils.doesStringContainArabic(mQuery);
    boolean showArabicWarning = (mIsArabicSearch &&
              !QuranFileUtils.hasArabicSearchDatabase(this));
      // If the flag showArabicWarning is true, then the Arabic database is not present or the search string is not arabic or both
      if (showArabicWarning) {
          mIsArabicSearch = false;
      }

      boolean showKanWarning = (mIsKanSearch &&
              !QuranFileUtils.hasKanSearchDatabase(this));
      // If the flag showKanWarning is true, then the Kannada database is not present or the search string is not Kannada or both
      if (showKanWarning) {
          mIsKanSearch = false;
      }
  ...
  private static class EfficientResultAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
      ...
      String text = v.text;
      String suraName = QuranInfo.getSuraNameOnly(mContext, v.sura, false);
      if (mShouldReshape) {
